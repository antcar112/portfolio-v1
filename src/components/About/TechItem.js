...
const StyledTechItem = styled.div`
  ...
  & img {
    height: 45px;
    filter: ${({ colorTheme, invert }) => (colorTheme === 'dark' && invert ? 'invert(1)' : '')} grayscale(100%);
    transition: 150ms filter linear;

    ${media.down.md} {
      filter: ${({ colorTheme, invert }) => (colorTheme === 'dark' && invert ? 'invert(1)' : '')} grayscale(0);
    }
    ${media.down.xs} {
      height: 35px;
    }
...
const TechItem = ({ tech }) => {
  ...
  return (
    <StyledTechItem colorTheme={theme.theme} invert={tech.invert}>
      ...
      {tech.inProgress && <span className='in_progress'>(In Progress)</span>}
    </StyledTechItem>
  )
}
